(function(){
  ...
  function renderCard(){
    ...
    // the interlocking pyramid on a white badge — the strategy's brand mark on the card
  ...
  // heart stays white; the pillar filter narrows the colored dots instead of recoloring the heart
  function recolorHeart(){ /* intentionally no-op — the heart is the white vessel that fills with color */ }
  ...
  // a pillar filter narrows to the teams that made a commitment in that pillar (matching the filtered dots)
  function filteredTeams(){
    var list = allTeams();
    if(board.filter==='all') return list;
    return list.filter(function(t){
      var cs=t.commitments||[];
      for(var i=0;i<cs.length;i++){ if(cs[i].goal===board.filter) return true; }
      return false;
    });
  }
  ...
  function renderPage(pg){
    ...
    if(g){
      g.innerHTML='';
      // one colored dot per commitment; a pillar filter shows only that color's dots
      var dots = dotSlice(pg);
      dots.forEach(function(d, i){ makeDot(g, d, true, (i%16)*0.04); });
    }
  }
  // the dots for the current view, as {team,text,goal,mine}:
  //   All  -> ONE dot per team (their first commitment), so each team shows once
  //   pillar -> that pillar's commitments (usually one per team)
  function boardDots(){
    var feed = board.feed, out=[];
    for(var i=0;i<feed.length;i++){
      var t=feed[i], cs=t.commitments||[]; if(!cs.length) continue;
      if(board.filter==='all'){
        out.push({ team:t.team, text:cs[0].text, goal:cs[0].goal, mine:!!t.mine });
      } else {
        for(var j=0;j<cs.length;j++){ if(cs[j].goal===board.filter) out.push({ team:t.team, text:cs[j].text, goal:cs[j].goal, mine:!!t.mine }); }
      }
    }
    return out;
  }
  // the dots to show for this page: a rotating window so the heart stays lively
  function dotSlice(pg){
    var all = boardDots();
    var n=all.length; if(!n) return [];
    var cap=Math.min(DOT_CAP, n), start=(pg*PAGE_SIZE)%n, out=[];
    for(var i=0;i<cap;i++){ out.push(all[(start+i)%n]); }
    return out;
  }
  ...
  function playBoardSequence(){
    ...
    var svg=mount.querySelector('svg'), g=mount.querySelector('#teamdots');
    var reduce=false; try{ reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches; }catch(e){}
    if(reduce){ if(g)g.style.opacity='1'; return; }
    if(svg){ svg.classList.remove('spinin'); void svg.offsetWidth; svg.classList.add('spinin'); }   // heart eases in
    setTimeout(function(){ if(g)g.style.opacity='1'; }, 850);                                        // then the teams fill it, and it stays a heart
  }

  // each dot is one commitment, colored by its pillar — the white heart fills up with color
  function makeDot(g, dot, pop, delay){
    var c = document.createElementNS(svgns,'circle');
    var isMine = !!dot.mine;
    var col = GOAL_DOTCOL[dot.goal] || HEART_COLOR;
    var x = isMine ? (108 + Math.random()*34) : (40 + Math.random()*172);
    var y = isMine ? (84  + Math.random()*26) : (46 + Math.random()*150);
    c.setAttribute('cx', x.toFixed(1)); c.setAttribute('cy', y.toFixed(1));
    c.setAttribute('class', isMine ? 'tdot mine' : 'tdot');
    c.setAttribute('fill', col);
    if(isMine){
      c.setAttribute('r','6.5');
      c.setAttribute('stroke','#ffffff'); c.setAttribute('stroke-width','2.4');   // white ring so your team stands out
      popAnim(c,'0;9.5;6.5','0.9s',0);
      if(pop) sonarRing(g, x, y, col);
    } else {
      c.setAttribute('stroke','#ffffff'); c.setAttribute('stroke-width','0.9');   // thin white halo separates colored dots
      c.setAttribute('opacity',(0.9+Math.random()*0.1).toFixed(2));
      var r = 2.6 + Math.random()*1.5;
      if(pop){ c.setAttribute('r','0'); popAnim(c,'0;'+(r+1.6).toFixed(1)+';'+r.toFixed(1),'0.6s', delay||0); } else c.setAttribute('r', r.toFixed(1));
    }
    c.setAttribute('data-team', (dot.team||'A Mission Team') + (isMine?' · your team':''));
    if(dot.text) c.setAttribute('data-commit', dot.text);
    if(dot.goal) c.setAttribute('data-goal', dot.goal);
    g.appendChild(c);
  }
  ...
  function sonarRing(g, x, y, col){
    ...
    ring.setAttribute('stroke', col || HEART_COLOR); ring.setAttribute('stroke-width','2');
